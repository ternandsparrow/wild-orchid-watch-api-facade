...
const wowConfig = {
  ...
  // the WOW project identifier. Slug is the fragment of URL, e.g. wow_project,
  // not the numeric ID
  inatProjectSlug:
  ...
  // iNat project as this role allows us to get unobscured GPS coordinates for
  // observations. This should be a dedicated user just for this use.
...
if (wowConfig.sentryDsn) {
  Sentry.init({
    dsn: wowConfig.sentryDsn,
...
;(() => { // eslint-disable-line no-extra-semi
  try {
    log.debug(`Asserting upload dir (${wowConfig.rootUploadDirPath}) exists`)
    const d = fs.opendirSync(wowConfig.rootUploadDirPath)
    d.close()
    log.debug(`Upload dir DOES exist`)
  } catch (err) {
    if (err.code === 'ENOENT') {
      log.warn('Upload dir does NOT exist, attemping to create...')
      fs.mkdirSync(wowConfig.rootUploadDirPath)
      log.warn('Upload dir successfully created')
      return
    }
    if (err.code === 'ENOTDIR') {
      throw new Error(`Upload dir path ${
        wowConfig.rootUploadDirPath} exists, but it NOT a directory, cannot continue`)
    }
    throw err
  }
})()
